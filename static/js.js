...
window.onload = function () {
    let temp_html = `        <div class="textBox">
        <p><span class="question">첫번째 질문</span>
            <br>어제 먹은 음식은 무엇인가요?<br>
            어제 먹은 음식을 제외하고 추천해드릴게요!</p>
    </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck1" autocomplete="off" checked name="category" value="한식">한식
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck2" autocomplete="off" checked name="category" value="중식">중식
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck3" autocomplete="off" checked name="category" value="일식">일식
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck4" autocomplete="off" checked name="category" value="양식">양식
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck5" autocomplete="off" checked name="category" value="분식">분식
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck6" autocomplete="off" checked name="category" value="디저트">빵
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck7" autocomplete="off" checked name="category" value="야식">야식
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck8" autocomplete="off" checked name="category" value="패스트푸드">패스트푸드
            </label>
        </div>
        <div class="btn-group-toggle" data-toggle="buttons">
            <label class="btn btn-secondary active">
                <input type="checkbox" class="btn-check" id="btncheck9" autocomplete="off" checked name="category" value="">아무거나 괜찮아!
            </label>
        </div>
    <div class="next"><button type="button" onclick="q2()" class="btnNext">클릭해 보세요!</button></div>`
...
function q2() {
    ...
    let category_length = document.getElementsByName("category").length;
    for (let i = 0; i < category_length; i++) {
        if (document.getElementsByName("category")[i].checked == true) {
            let btn_check_val = document.getElementsByName("category")[i].value;

            $.ajax({
                type: "POST",
                url: "/Q_2",
                data: {q2_give: btn_check_val},
